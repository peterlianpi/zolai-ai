...
LOG.write_text("")
log("\n" + "="*80)
log("BUILDING MASTER DATASET - OPTION 2 (Training + Dictionary)")
log("="*80 + "\n")

# Stage 1: Collect files
log("[1/4] COLLECTING FILES...")
files = (
...
out_all = DATA / "training/master_unified_all.jsonl"
...
with open(DATA / "training/master_train.jsonl", "w", encoding="utf-8") as f:
    for rec in records[:train_n]:
        f.write(json.dumps(rec, ensure_ascii=False) + "\n")
log(f"  ✓ master_train.jsonl ({train_n:,} records)")

with open(DATA / "training/master_val.jsonl", "w", encoding="utf-8") as f:
    for rec in records[train_n:train_n+val_n]:
        f.write(json.dumps(rec, ensure_ascii=False) + "\n")
log(f"  ✓ master_val.jsonl ({val_n:,} records)")

with open(DATA / "training/master_test.jsonl", "w", encoding="utf-8") as f:
    for rec in records[train_n+val_n:]:
        f.write(json.dumps(rec, ensure_ascii=False) + "\n")
log(f"  ✓ master_test.jsonl ({test_n:,} records)\n")

# Create manifest
manifest = {
    "generated_at": datetime.now().isoformat(),
    "option": 2,
    "description": "Training + Dictionary (recommended)",
