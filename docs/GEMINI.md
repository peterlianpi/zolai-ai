...
## Project Overview
**Zomi AI** (Zolai Second Brain) is a production-grade NLP toolkit and ecosystem dedicated to the **Tedim Zolai** language (Zolai Standard). It encompasses a full pipeline for data collection, cleaning, linguistic analysis, instruction synthesis, and LLM fine-tuning.
...
- **Primary Standard:** **Tedim Zolai Standard** (Zolai Vernacular Standard 2018).
...
- `python scripts/data_pipeline/build_master_dataset.py` — Compile the master training set.
- `python scripts/synthesize_instructions_v6.py` — Generate synthetic instruction data.
- `python scripts/training/train_llm.py` — Fine-tune models (typically on Kaggle).
...
## Linguistic Mandates (Strict Tedim Zolai Standard)
...
- **Standardization:** All Zolai text must pass through the `Zolai Standard` cleaner/standardizer.
...
```bash
gemini -p "$(cat GEMINI.md)" "Is this Zolai sentence grammatically correct Tedim Zolai Standard? Explain: 'Ka nu in anlim a huan hi.'"
```
...
- Eval: `data/eval/` (QA, translation, Zolai Standard compliance benchmarks)
