...
pip install -r requirements.txt      # Core ML deps (CPU-optimized for 4 cores, 7.7GB RAM)
...
- Use `zolai.utils.data_loader` for JSONL streaming

### Memory Optimization (CPU-only, 7.7GB RAM)
- Use `gradient_checkpointing=True` in training configs
- Use `gradient_accumulation_steps=4` to reduce batch size
- Stream data with `batch_stream_jsonl()` instead of loading full datasets
- Use `torch.set_num_threads(4)` to optimize CPU threading
- Avoid loading entire 20GB data/ directory into memory
- Use `accelerate` with `config/cpu_optimization.yaml` for multi-core training
...
- **Difficulty Control (CEFR Aligned):** Adjust based on level:
  - **A1-A2 (Beginner):** Focus on SOV/OSV identity, simple prepositions, and past/future markers (khin, ding, ngei). Teach `Tua ciangin` for narrative sequence and the day-counting formula from Genesis.
  - **B1-B2 (Intermediate):** Focus on cause/effect (ahih manin), interrogatives, conditional logic (leh), and quotative patterns (`ci hi`, `ci-in`). Introduce directional verb particles (hong, va, khia, lut, kik).
  - **C1-C2 (Advanced/Mastery):** Use embedded clauses, I Am metaphors (`KEIMAH ka hi hi`), rhetorical questions, poetic parallelism, and complex biblical narrative connectors (`Tua ahih ciangin`, `Bang hang hiam cih leh`).
...
- **Register Awareness:** Distinguish formal (biblical/written) from informal (conversational) register. In informal speech, subject pronouns are commonly dropped when the agreement marker (`ka`, `na`, `a`) makes the person clear. E.g., `An ka ne hi.` (informal) vs `Kei in an ka ne hi.` (formal).
- **Quotative Patterns:** Teach `ci hi` (said), `ci-in` (saying/having said), and `kici` (is called) as essential B1 patterns for reported and direct speech.
- **Directional Particles:** At A2+, teach `hong` (toward speaker), `va` (away), `khia` (out), `lut` (in), `kik` (back), `pih` (send/bring) as verb modifiers that change movement meaning.
