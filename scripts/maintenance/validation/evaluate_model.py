...
FORBIDDEN = {"pasian", "ram", "tapa", "topa", "tua", "tua", "kumpipa"}
...
def run_zvs_eval(test_file="data/eval/zvs_compliance_test_v1.jsonl"):
    ...
    print(f"\n=== Zolai Standard Compliance Test ({len(tests)} pairs) ===")
    ...
    print(f"Gold 'correct' pass Zolai Standard: {correct_pass}/{len(tests)} ({correct_pass/len(tests):.1%})")
    print(f"Gold 'wrong'   fail Zolai Standard: {wrong_fail}/{len(tests)} ({wrong_fail/len(tests):.1%})")
...
def show_stats():
    files = {
        "Zolai Standard compliance test": "data/eval/zvs_compliance_test_v1.jsonl",
