...
2. Prefer Tedim2010 (Zolai Standard) over TDB77 for headwords
...
NEGATION_NOTES = {
    "kei": (
        "Standard Zolai Standard negation. "
    ...
    "lo": (
        "Valid Zolai Standard negation, common in 3rd person: 'Amah dam lo hi' (he is not well). "
...
def study_corpus(bible_dir):
    ...
    Read every Bible book and build:
      ...
      tdb_to_tdm[word] = TDB77 word → Tedim2010 equivalent (Zolai Standard correction)
...
def build_entries(zo_en, zo_zo, zo_neg, zo_examples, tdb_to_tdm, book_count, total_verses):
    ...
    # Use only the hardcoded Zolai Standard corrections.
    ...
    for word, en_counter in sorted(zo_en.items()):
        ...
        if zvs_fix:
            usage += f" ⚠ Non-Zolai Standard: use '{zvs_fix}' instead."
