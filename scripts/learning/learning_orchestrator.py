...
ZVS_RULES = {
    'forbidden_words': ['pasian', 'tapa', 'topa', 'kumpipa', 'ram', 'tua', 'tua'],
    ...
    async def run(self, cycle: LearningCycle):
        ...
        try:
            ...
            for en, expected_zo in test_cases[:3]:  # Test 3 per cycle
                prompt = f"Is this Tedim Zolai Standard correct? '{expected_zo}' for '{en}'. Reply: CORRECT or WRONG: [correction]"
                try:
...
async def main():
    ...
    logger.info(f"Zolai Standard Rules: {json.dumps(ZVS_RULES, indent=2)}")
