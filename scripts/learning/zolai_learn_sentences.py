...
AGENTS = {
    "linguistic-specialist": (
        "You are a Tedim Zolai linguistic specialist (Zolai Standard). "
        "Analyze morphology, ergative 'in', directional 'hong', SOV structure (allowing for natural object-fronting), and Zolai Standard compliance. "
...
CURATED = [
    ...
    ("Ken laibu ka sim hi.", "Ergative — Ken (Kei+in) I read a book"),
    ("Nangin na pai hi.", "Ergative — Nangin (Nang+in) you go"),
...
def load_wiki_sentences(n: int = 10) -> list[tuple[str, str]]:
    """Extract example sentences from wiki markdown files."""
    results: list[tuple[str, str]] = []
    grammar_dir = ROOT / "wiki"
    sentence_patterns = [
        re.compile(r'^\s*\\*\\*(`[^`]+`|"[^"]+")\\*\\*'),  # Bolded quoted strings
        re.compile(r'^\s*(`[^`]+`|"[^"]+")'),              # Plain quoted strings
        re.compile(r'^\s*(?:- )?(`[^`]+`|"[^"]+")'),       # List items with quoted strings
        re.compile(r"^\\s*([A-Z].*[.!?])"),                # Sentences starting with capital letter
        re.compile(r"^\\s*([A-Z].*[.!?])\\s*(?:—|=)\\s*"), # Example with translation like "Sentence. = Translation"
    ]
    code_block_pattern = re.compile(r"```(?:zolai|python)?\\n(.*?)\\n```", re.DOTALL)
    ...
    for md_file in grammar_dir.glob("**/*.md"):
        ...
        for block_match in code_block_pattern.finditer(text):
            ...
            for line in block_content.splitlines():
                line = line.strip()
                if 15 < len(line) < 120 and any(w in line for w in ["hi", "uh", "in ", "ka ", "a "]):
                    results.append((line, f"wiki/{md_file.name}"))

        # Then extract from regular lines
        for line in text.splitlines():
            line = line.strip()
            if line.startswith("| ") or line.startswith("|"):  # Handle markdown tables
                parts = [p.strip() for p in line.split("|") if p.strip()]
                for part in parts:
                    part = part.strip("*`_ ")
                    if (
                        15 < len(part) < 120
                        and any(w in part for w in ["hi", "uh", "in ", "ka ", "a "])
                ...
                if len(results) >= n * 3:
                    break
            else:  # Handle other line patterns
                for pattern in sentence_patterns:
                    ...
                    if match:
                        ...
                        if (
                            15 < len(sentence) < 120
                            and any(w in sentence for w in ["hi", "uh", "in ", "ka ", "a "])
                            and not sentence.startswith("#")
                            and not sentence.startswith("---")
                        ):
                            results.append((sentence, f"wiki/{md_file.name}"))
                            if len(results) >= n * 3:
                                break
                if len(results) >= n * 3:
                    break
    ...
    filtered = [(s, src) for s, src in results if any(c.islower() for c in s) and len(s.split()) >= 3]
