...
SYSTEM = (
    ...
    "You teach, translate, and explain Tedim Zolai using the Zolai Standard. "
    "Always use: pasian, gam, tapa, topa, kumpipa, tua. "
    "Never use: pathian, ram, fapa, bawipa, siangpahrang, cu/cun."
)
    ...
    with open(path, encoding="utf-8") as f:
        for line in f:
            ...
            if not zo or not en or not ok_text(zo) or not ok_instr(r.get("instruction", "")):
                continue
            emit(fout, f"Translate the following English text into Tedim Zolai (Zolai Standard).\n{en}", zo)
