    ...
    def fix_wiki_examples(self) -> Dict:
        ...
        for md_file in wiki_root.rglob("*.md"):
            try:
                ...
                # Fix plural markers - never combine 'uh' with first-person inclusive 'i'
                content = re.sub(r'\bi\s+([a-zA-Z\s]{1,50})\buh\b\s+(hi|hih|uh hi|uh hi)\b', r'i \1 \2', content)
                content = re.sub(r'\bi\s+([a-zA-Z\s]{1,50})\buh\b', r'i \1', content)
                
                # Fix common typos and spacing
                content = re.sub(r'\buh hi\b', 'uh hi', content)
                content = re.sub(r'\bahi hi\b', 'ahi hi', content)
                content = re.sub(r'\btani\b', 'tanu', content)
                
                # Fix word order - ensure SOV (basic check)
