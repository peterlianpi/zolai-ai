...
export async function sendWelcomeEmail(name: string, email: string) {
  try {
    // Check site setting — skip if disabled
    const { getSiteSetting } = await import("@/lib/site-config");
    const sendWelcome = await getSiteSetting("send_welcome_email", "true");
    if (sendWelcome === "false") return { success: true, skipped: true };

    const { subject, html } = emailTemplates.welcome({ name, email });
    const result = await sendEmailViaResend({
      to: email,
      subject: `[Action Required] ${subject}`,
      html,
    });
    const messageId = (result as { messageId?: string }).messageId ?? (result as { id?: string }).id;
    return { success: true, messageId };
  } catch (error) {
