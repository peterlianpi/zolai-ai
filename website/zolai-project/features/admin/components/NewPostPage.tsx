...
export function NewPostPage() {
  ...
  const [form, setForm] = useState({ title: "", slug: "", type: "POST" as const, status: "DRAFT" as const, contentHtml: "", excerpt: "" });

  async function save() {
    if (!form.title || !form.slug) { toast.error("Title and slug required"); return; }
    setSaving(true);
    try {
      const res = await client.api.content.posts.$post({ json: { ...form } });
