...
export function AdminRoleManagementPage() {
  ...
  const users: AdminUser[] = usersQuery.data && 'data' in usersQuery.data && usersQuery.data.success
    ? (usersQuery.data.data as AdminUser[]) ?? []
    : [];
  const metricsData = metricsQuery.data && 'data' in metricsQuery.data && metricsQuery.data.success
    ? metricsQuery.data.data
    : null;
  const roleDistribution = useMemo(() => metricsData?.roleDistribution ?? {}, [metricsData]);
  const recentChanges: RoleChange[] = (metricsData?.recentRoleChanges ?? []) as RoleChange[];
                ...
                <div key={change.id} className="flex flex-col justify-between gap-2 rounded-md border p-3 sm:flex-row sm:items-center">
                  <div className="space-y-1">
                    <div className="text-sm font-medium">
                      {change.affectedUser?.name ?? "Unknown user"}
                      {" "}
                      <span className="text-muted-foreground">{change.affectedUser?.email}</span>
                    </div>
