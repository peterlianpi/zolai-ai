...
export default function EditWikiPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    client.api.zolai.wiki[":slug"].$get({ param: { slug: id } })
      .then(r => r.json())
      .then(d => {
        const data = (d as { data?: { title: string; category: string; status: string; content: string } }).data;
        if (data) setForm({ title: data.title, category: data.category, status: data.status, content: data.content });
      });
  }, [id]);
