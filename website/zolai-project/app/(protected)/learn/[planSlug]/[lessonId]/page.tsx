...
export default function LessonPage({ params }: { params: Promise<{ planSlug: string; lessonId: string }> }) {
  const { planSlug, lessonId } = use(params);
  ...
  async function handleComplete(score: number) {
    ...
    if (userId && lesson) {
      ...
      setXpEarned(res?.xpEarned ?? lesson.xpReward);
    }
    setPhase("result");
  }
