...
const roleManagement = new Hono()
  ...
  .get(
    ...
    async (c) => {
      ...
      try {
        ...
        // Get role change events from audit log (last 30 days)
        const thirtyDaysAgo = new Date();
        thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);

        const roleChanges = await prisma.auditLog.count({
          where: {
            action: "UPDATE",
            entityType: "User",
            createdAt: {
        ...
        const recentRoleChanges = await prisma.auditLog.findMany({
          where: {
            action: "UPDATE",
            entityType: "User",
            createdAt: {
  ...
  .post(
    ...
    async (c) => {
      ...
      const { userIds, role, reason } = c.req.valid("json");
